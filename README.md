...
La GUI rileva dinamicamente le lingue presenti nel JSON e mostra come selezionabili solo quelle supportate da almeno un indice. L'infrastruttura di selezione e pipeline supporta già N lingue senza modifiche alla GUI. La presenza del supporto sillabico per una lingua non implica automaticamente che tutti gli indici siano applicabili: ogni estensione richiede un adattamento metodologicamente appropriato. Per Flesch, gli adattamenti che mantengono la struttura `C - asl * ASL - asw * ASW` sono configurati tramite `FLESCH_PARAMS` in `src/indices.py`; la funzione usa una sola formula generica con parametri specifici per lingua (`it`, `en`, `fr`). Aggiungere il supporto Flesch per una nuova lingua richiede una voce in `FLESCH_PARAMS` con coefficienti verificati, oltre all'aggiornamento di `INDEX_LANGS`. Una lingua non riconosciuta passata direttamente alle funzioni di `src/utils.py` solleva un `ValueError` esplicito; il flusso normale della pipeline è protetto a monte dal guard `INDEX_LANGS` in `_collect_results`. La sillabazione francese è stata validata su un campione Wiktionnaire e sul corpus reale a 4 lingue; restano casi noti non corretti dall'implementazione attuale (`connaissance`, `Constantinople`, `aujourd'hui`), riconducibili ai risultati prodotti da pyphen/Hunspell e non alle correzioni euristiche introdotte. Lo spagnolo ha il supporto sillabico ma non ha ancora un adattamento Flesch implementato.
